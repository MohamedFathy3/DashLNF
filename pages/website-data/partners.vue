...
definePageMeta({
    ...
    permissions: ['show-website-data-partners'],
});

// ========== Permissions ==========
const pageSlug = 'website-data-partners';
            ...
            <table class="table table-report font-light w-full">
                ...
                <tbody>
                    ...
                    <tr v-if="!pending && rows?.data?.length === 0">
                        <td colspan="5" class="p-8 text-center text-sm text-slate-500">No partners found.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <TheModal :open-modal="isOpen" size="5xl" @close-modal="closeModal()">
            <template #header>
                <div class="flex justify-between items-center">
                    <div>
                        <div class="text-lg font-semibold text-slate-800">{{ editMode ? 'Update Partner' : 'Add New Partner' }}</div>
                        <div class="text-xs text-slate-500">{{ editMode ? 'Edit partner details' : 'Create a new partner' }}</div>
                    </div>
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    ...
                    <div class="lg:col-span-8 grid lg:grid-cols-12 gap-5">
                        <FormInputField v-model="item.name" :errors="v$.name.$errors" class="lg:col-span-12" label="Name" name="name" placeholder="Partner Name" />
                        <FormInputField v-model="item.link" :errors="v$.link.$errors" class="lg:col-span-12" label="Link" name="link" type="url" placeholder="https://example.com" />
