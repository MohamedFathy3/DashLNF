...
definePageMeta({
    ...
    permissions: ['show-website-data-services'],
});

// ========== Permissions ==========
const pageSlug = 'website-data-services';
    ...
    <div class="flex flex-col gap-8">
        ...
        <div class="md:flex md:items-center md:justify-between md:gap-5">
            <div class="flex items-center gap-2">
                <Icon name="solar:star-circle-linear" class="size-5 opacity-75" />
                ...
                <tbody>
                    <template v-if="status !== 'pending' && rows">
                        <tr v-for="row in rows.data" :key="row.id" class="border-b hover:bg-slate-50/50">
                            ...
                            <td>
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="font-light text-xs opacity-75 mt-0.5">{{ row.slug }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="text-center">
                                <FormSwitch :id="'row-active-' + row.id" v-model="row.active" :disabled="serverParams.deleted" @change="useToggleSwitch(row.id, 'active', 'service')" />
                            </td>
                    ...
                    <tr v-if="status !== 'pending' && rows?.data?.length === 0">
                        <td colspan="6" class="p-8 text-center text-sm text-slate-500">No services found.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <TheModal :open-modal="isOpen" size="5xl" @close-modal="closeModal()">
            <template #header>
                <div class="flex justify-between items-center">
                    <div>
                        <div class="text-lg font-semibold text-slate-800">{{ editMode ? 'Update Service' : 'Add New Service' }}</div>
                        <div class="text-xs text-slate-500">{{ editMode ? 'Edit service details' : 'Create a new service' }}</div>
                    </div>
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    ...
                    <div class="lg:col-span-8 grid grid-cols-12 gap-5">
                        <FormInputField v-model="item.name" :errors="v$.name.$errors" class="lg:col-span-12" label="Name" name="name" placeholder="Service Name" />
                        <FormInputField v-model="item.icon" :errors="v$.icon.$errors" class="lg:col-span-12" label="Icon Name" name="icon" placeholder="Icon Name (e.g., solar:star-circle-linear)" />
                    ...
                    <!-- Short Description -->
                    <FormInputField v-model="item.shortDescription" :errors="v$.shortDescription.$errors" class="lg:col-span-12" label="Short Description" name="shortDes" placeholder="Short Description" type="textarea" rows="3" />
