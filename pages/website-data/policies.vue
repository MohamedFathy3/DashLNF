...
definePageMeta({
    ...
    permissions: ['show-website-data-policies'],
});

// ========== Permissions ==========
const pageSlug = 'website-data-policies';
            ...
            <table class="table table-report font-light w-full">
                ...
                <tbody>
                    ...
                    <tr v-if="!pending && rows?.data?.length === 0">
                        <td colspan="6" class="p-8 text-center text-sm text-slate-500">No policies found.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <TheModal :open-modal="isOpen" size="5xl" @close-modal="closeModal()">
            <template #header>
                <div class="flex justify-between items-center">
                    <div>
                        <div class="text-lg font-semibold text-slate-800">{{ editMode ? 'Update Policy' : 'Add New Policy' }}</div>
                        <div class="text-xs text-slate-500">{{ editMode ? 'Edit policy details' : 'Create a new policy' }}</div>
                    </div>
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    <FormInputField v-model="item.title" :errors="v$.title.$errors" class="lg:col-span-12" label="Title" name="title" placeholder="Policy title" />
