...
const {
    ...
    pending,
    refresh,
} = await useApiFetch('/api/policy/index', {
    method: 'POST',
    body: serverParams,
    lazy: true,
});
...
const fetchItem = async (id) => {
    const { data, error } = await useApiFetch(`/api/policy/${id}`, {
        lazy: true,
    });
    if (data.value) {
...
async function updateItem() {
    const { data, error } = await useApiFetch(`/api/policy/${item.value?.id}`, {
        method: 'PATCH',
        body: item,
        lazy: true,
    });
...
async function addItem() {
    const { data, error } = await useApiFetch(`/api/policy`, {
        method: 'POST',
        body: item,
        lazy: true,
    });
...
async function deleteItems() {
    ...
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/policy/delete`, {
...
async function forceDeleteItems() {
    const confirmed = confirm('Are you sure you want to delete this item?');
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/policy/force-delete`, {
            body: { items: selectedRows.value },
            method: 'DELETE',
            lazy: true,
        });
...
async function restoreItems() {
    const confirmed = confirm('Are you sure you want to delete this item?');
    if (confirmed) {
        const { data, error } = await useApiFetch(`/api/policy/restore`, {
            body: { items: selectedRows.value },
            method: 'POST',
            lazy: true,
        });
    ...
    <div class="flex flex-col gap-8">
        ...
        <div class="md:flex md:items-center md:justify-between md:gap-5">
            <div class="flex items-center gap-2">
                ...
                <div>{{ serverParams.deleted ? 'Deleted Policies' : 'Policies' }}</div>
            </div>
            <div class="md:flex md:items-center md:gap-5 md:space-y-0 space-y-5">
                <template v-if="selectedRows.length > 0">
                    <button v-if="serverParams.deleted && useCheckPermission(['force-delete-policy'])" class="btn btn-danger btn-rounded px-6 btn-sm gap-3 md:w-fit w-full md:mt-0 mt-5" @click="forceDeleteItems">
                        <Icon name="solar:trash-bin-minimalistic-line-duotone" class="size-5 opacity-75" />
                        Delete Permanently
                    </button>
                    <button v-else-if="!serverParams.deleted && useCheckPermission(['delete-policy'])" class="btn btn-danger btn-rounded px-6 btn-sm gap-3 md:w-fit w-full md:mt-0 mt-5" @click="deleteItems">
                        <Icon name="solar:trash-bin-minimalistic-line-duotone" class="size-5 opacity-75" />
                        Delete Items
                    </button>
                    <button v-if="serverParams.deleted && useCheckPermission(['restore-policy'])" class="btn btn-success btn-rounded px-6 btn-sm gap-3 md:w-fit w-full md:mt-0 mt-5" @click="restoreItems">
                        <Icon name="solar:restart-circle-outline" class="size-5 opacity-75" />
                        Restore Items
                    </button>
                </template>
                <button v-if="useCheckPermission(['create-policy'])" :disabled="serverParams.deleted" class="btn btn-primary btn-rounded px-6 btn-sm gap-3 md:w-fit w-full md:mt-0 mt-5" @click="openModal()">
            ...
            <tbody>
                <template v-if="!pending && rows">
                    <tr v-for="row in rows.data" :key="row.id" class="text-sm">
                        ...
                        <td class="text-right">
                            <div>
                                <button v-if="useCheckPermission(['edit-policy'])" :disabled="serverParams.deleted" class="btn btn-secondary btn-rounded btn-sm gap-3" @click="openModal(row.id)">
