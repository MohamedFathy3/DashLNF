...
definePageMeta({
    ...
    permissions: ['show-website-data-sections'],
});
...
// ========== Permissions ==========
const pageSlug = 'website-data-sections';
            ...
            <table class="table table-report font-light w-full">
                ...
                <tbody>
                    ...
                    <tr v-if="!pending && rows?.data?.length === 0">
                        <td colspan="5" class="p-8 text-center text-sm text-slate-500">No sections found.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <TheModal :open-modal="isOpen" size="5xl" @close-modal="closeModal()">
            <template #header>
                <div class="flex justify-between items-center">
                    <div>
                        <div class="text-lg font-semibold text-slate-800">{{ editMode ? 'Update Section' : 'Add New Section' }}</div>
                        <div class="text-xs text-slate-500">{{ editMode ? 'Edit section details' : 'Create a new section' }}</div>
                    </div>
