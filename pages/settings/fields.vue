...
definePageMeta({
    ...
    permissions: ['show-settings-fields'],
});

// ========== Permissions ==========
const pageSlug = 'settings-fields';
            ...
            <table class="table table-report font-light w-full">
                ...
                <tbody>
                        ...
                        <tr v-for="row in rows.data" :key="row.id" class="border-b hover:bg-slate-50/50">
                            ...
                            <td class="text-center">
                                <span :class="[row.type === 'section' ? 'bg-success/25 text-success !border-success/25' : 'bg-slate-100 text-slate-700 !border-slate-200', 'rounded-full border px-3 py-1 text-xs font-medium']">
                                    {{ fieldTypes.find((f) => f.id === row.type)?.name || row.type }}
                                </span>
                            </td>
                            <td class="text-center">
                    ...
                    <tr v-if="!pending && rows?.data?.length === 0">
                        <td colspan="6" class="p-8 text-center text-sm text-slate-500">No setting fields found.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <TheModal :open-modal="isOpen" size="5xl" @close-modal="closeModal()">
            <template #header>
                <div class="flex justify-between items-center">
                    <div>
                        <div class="text-lg font-semibold text-slate-800">{{ editMode ? 'Update Setting Field' : 'Add New Setting Field' }}</div>
                        <div class="text-xs text-slate-500">{{ editMode ? 'Edit field details' : 'Create a new setting field' }}</div>
                    </div>
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    <!-- Basic Info -->
                    <FormInputField v-model="item.label" :errors="v$.label.$errors" class="lg:col-span-6" label="Label" name="label" placeholder="Field Label" />
                    <FormSelectField v-model="item.type" :errors="v$.type.$errors" labelvalue="name" keyvalue="id" :select-data="fieldTypes" class="lg:col-span-6" label="Type" name="type" placeholder="Select field type" />

                    <!-- Field Details (non-section) -->
                    <template v-if="item.type && item.type !== 'section'">
                        ...
                        />
                        <FormInputField v-model="item.des" :errors="v$.des.$errors" class="lg:col-span-12" label="Description" name="des" placeholder="Field description" type="textarea" rows="3" />
                        <FormInputField v-model="item.data" :errors="v$.data.$errors" class="lg:col-span-4" label="Data" name="data" placeholder="Data (for select/list)" />
                        <FormInputField v-model="item.name" :errors="v$.name.$errors" class="lg:col-span-4" label="Name" name="name" placeholder="Field name" />
                        <FormInputField v-model="item.class" :errors="v$.class.$errors" class="lg:col-span-4" label="Class" name="class" placeholder="CSS class" />
                        <FormInputField v-model="item.placeholder" :errors="v$.placeholder.$errors" class="lg:col-span-6" label="Placeholder" name="placeholder" placeholder="Placeholder text" />

                        <!-- Uploader Type -->
                        <template v-if="item.type === 'uploader'">
                            <FormUploader v-model="item.image" :errors="v$.image.$errors" :allowed-types="['image', 'svg']" class="lg:col-span-12" label="Default Image" name="image" />
                        </template>
                        <template v-else>
                            <FormInputField v-model="item.value" :errors="v$.value.$errors" class="lg:col-span-6" label="Default Value" name="value" placeholder="Default value" />
                        </template>
                    </template>

                    <!-- Section Type Message -->
                    <div v-else-if="item.type === 'section'" class="lg:col-span-12 p-4 bg-slate-50 rounded-xl border border-dashed">
                        <div class="flex items-center gap-2 text-slate-500">
                            <Icon name="solar:info-circle-outline" class="size-5" />
                            <span class="text-sm">Section type does not require additional fields. It will be used as a group container.</span>
                        </div>
                    </div>
                </div>
            </template>
