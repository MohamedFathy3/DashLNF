...
definePageMeta({
    ...
    permissions: ['show-settings-email-templates'],
});

// ========== Permissions ==========
const pageSlug = 'settings-email-templates';
            ...
            <table class="table table-report font-light w-full">
                ...
                <tbody>
                    ...
                    <tr v-if="status !== 'pending' && rows?.data?.length === 0">
                        <td colspan="5" class="p-8 text-center text-sm text-slate-500">No email templates found.</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <TheModal :open-modal="isOpen" size="5xl" @close-modal="closeModal()">
            <template #header>
                <div class="flex justify-between items-center">
                    <div>
                        <div class="text-lg font-semibold text-slate-800">{{ editMode ? 'Update Email Template' : 'Add New Email Template' }}</div>
                        <div class="text-xs text-slate-500">{{ editMode ? 'Edit template details' : 'Create a new email template' }}</div>
                    </div>
            ...
            <template #content>
                <div class="grid lg:grid-cols-12 gap-5 items-start">
                    <FormInputField v-model="item.name" :errors="v$.name.$errors" class="lg:col-span-6" label="Name" name="name" placeholder="Template Name" />
                    <FormInputField v-model="item.slug" :errors="v$.slug.$errors" class="lg:col-span-6" label="Slug" name="slug" placeholder="template-slug" />
                    <FormInputField v-model="item.subject" :errors="v$.subject.$errors" class="lg:col-span-12" label="Subject" name="subject" placeholder="Email Subject" />
                    <FormInputField v-model="item.bcc" :errors="v$.bcc.$errors" class="lg:col-span-12" label="BCC" name="bcc" placeholder="bcc@example.com (comma separated)" />
                    <FormRichTextEditor v-model="item.body" :errors="v$.body.$errors" label="Message Body" class="lg:col-span-12" />
                </div>
            </template>
            <template #footer>
                <div class="w-full flex items-center justify-end gap-5">
                    <div class="mr-auto">
                        <a v-if="showExamplesLinks.find((e) => e.slug === item.slug)?.path" :href="config.public.apiUrl + showExamplesLinks.find((e) => e.slug === item.slug)?.path" target="_blank">
                            <button class="btn-rounded btn-sm btn btn-success px-4" type="button">
                                <Icon name="solar:eye-linear" class="w-5 h-5 mr-2" />
