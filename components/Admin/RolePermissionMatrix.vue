...
const groups = computed(() => {
    ...
    let current: { id: string; name: string; permissions: Permission[] } | null = null;

    for (const permission of flattenPermissions.value) {
        if (!isActionPermission(permission)) {
            current = { id: permission.slug, name: permission.name, permissions: [] };
            result.push(current);
            continue;
        }

        if (!current) {
            const resource = permission.slug.replace(/^(list|create|edit|delete|restore|force-delete)-/, '');
            current = { id: resource, name: resource.replaceAll('-', ' '), permissions: [] };
            result.push(current);
        }
