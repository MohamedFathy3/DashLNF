...
const fetchItem = async (id) => {
    ...
    if (data.value?.result === 'Success' || data.value?.data) {
        ...
        const memberNetwork = personData.network || personData.memberNetwork || null;
        item.value = {
            id: personData.id || null,
            title: personData.title || null,
            member_network_id: memberNetwork?.id || personData.member_network_id || null,
            memberNetwork,
            image: personData.image || null,
            name: personData.name || fullName || null,
            job_title: personData.jobTitle || personData.job_title || null,
            phone: personData.phoneNumber || personData.phone || null,
            cell_number: personData.cellNumber || personData.cell_number || null,
            email: personData.email || null,
            birth_date: personData.birthDate || personData.birth_date || null,
            phone_key_id: personData.phoneKeyId || personData.phone_key_id || null,
            phone_key: personData.cellKeyId || personData.cell_key_id || null,
                ...
                <div class="lg:col-span-4">
                    ...
                    <div v-if="isEditMode && item.memberNetwork" class="mt-4 p-3 bg-slate-50 rounded-xl">
                        ...
                        <div class="text-xs text-slate-500">
                            {{ item.memberNetwork.city }}{{ item.memberNetwork.country?.name ? `, ${item.memberNetwork.country.name}` : item.memberNetwork.countryId ? `, ID: ${item.memberNetwork.countryId}` : '' }}
                        </div>
